"""https://github.com/cmendl/pytenet/blob/master/experiments/evolution_operator.py"""
...
def schmidt_coefficients_wavefunction(d: int, L: int, psi: np.ndarray):
    ...
    return np.linalg.svd(
        psi.reshape((d ** (L // 2), d ** (L // 2))), full_matrices=0, compute_uv=False
    )
...
def schmidt_coefficients_operator(d: int, L: int, op: np.ndarray):
    ...
    """
    op = (
        op.reshape((d ** (L // 2), d ** (L // 2), d ** (L // 2), d ** (L // 2)))
        .transpose((0, 2, 1, 3))
        .reshape(d**L, d**L)
    )
...
def main():
    ...
    print(
        "[H, .] applied to H as vector (should be zero):",
    ...
    Dmax = 40
    D = np.minimum(
        np.minimum(d ** (2 * np.arange(L + 1)), d ** (2 * (L - np.arange(L + 1)))), Dmax
    )
    print("D:", D)
    ...
    comm = (
        (mpoHcomm.as_matrix() @ psi.as_vector())
        .reshape((2 * L) * [d])
        .transpose(
            np.concatenate(
                (2 * np.arange(L, dtype=int), 2 * np.arange(L, dtype=int) + 1)
            )
        )
        .reshape((d**L, d**L))
    )
    ...
    print(
        ...
        np.linalg.norm(
            sigma_0 - schmidt_coefficients_wavefunction(d**2, L, psi.as_vector())
        ),
    )
    ...
    plt.ylabel(r"$\sigma_i$")
    plt.title(
        f"Schmidt coefficients of time-evolved state (t = {-1j*t:g})\n(based on exact time evolution)"
    )
    ...
    print(
        "np.linalg.norm(P @ mpsH.as_vector() - mpsH.as_vector()) (in general non-zero):",
    ...
    plt.title(
        f"TDVP time evolution (applied to operator) rate of convergence for\nHeisenberg XXZ model (J={J:g}, D={DH:g}, h={h:g}), L={L}, t={-1j*t:g}"
    )
    ...
    plt.title(
        f"TDVP time evolution (applied to operator) relative energy error for\nHeisenberg XXZ model (J={J:g}, D={DH:g}, h={h:g}), L={L}, t={-1j*t:g}"
    )
...
def heisenberg_xxz_comm_mpo(L, J, D, h):
    ...
    qd = np.array([1, -1])
    # spin operators
    Sup = np.array([[0.0, 1.0], [0.0, 0.0]])
    Sdn = np.array([[0.0, 0.0], [1.0, 0.0]])
